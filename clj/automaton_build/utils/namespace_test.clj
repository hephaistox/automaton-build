...
(deftest symbol-to-fn-call-test
  (testing "Existing functions are called"
    (is (= ["foo" "bar"]
           (sut/symbol-to-fn-call 'automaton-build.utils.namespace-test/foo
                                  "bar"))))
  (testing "Non existing functions are ok"
    (is (nil? (sut/symbol-to-fn-call 'automaton-build.utils.namespace-test/foo2
                                     "bar"))))
  (testing "Non existing namespaces are ok"
    (is (nil? (sut/symbol-to-fn-call 'automaton-build.utils.namespace-test2/foo
                                     "bar"))))
  (testing "Exceptions during function call returns with nil"
    (is (nil? (sut/symbol-to-fn-call
               'automaton-build.utils.namespace-test/foo-ex
               "on purpose ex")))))
