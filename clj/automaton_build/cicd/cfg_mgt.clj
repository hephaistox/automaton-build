...
(defn- replace-files-between-repos
  [source-dir tmp-dir]
  (let [source-dir-files (->> (build-files/search-files source-dir "*")
                              (filter (fn [file]
                                        (not (str/ends-with? file ".git")))))
        tmp-dir-files (->> (build-files/search-files tmp-dir "*")
                           (filter (fn [file]
                                     (not (str/ends-with? file ".git")))))]
...
(defn- squash-local-files-and-push
  ...
  * `force?` (optional)"
  ...
  ([tmp-dir source-dir commit-message tag force?]
   (when (git-installed?)
     (when (replace-files-between-repos source-dir tmp-dir)
       (if tag
...
(defn push-local-dir-to-repo
  ...
  * `target-branch` (optional default current-branch) where to push"
  ([{:keys [source-dir
   ...
   (when (git-installed?)
     (build-log/debug "Pushing from: `%s` to repository" source-dir)
