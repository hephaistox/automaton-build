...
(defn run-monorepo
  ...
  (let [app-dir ""
        ...
        commit-msg (if-let [commit-msg (get-in cli-opts
    ...
    (if (get-in monorepo-project-map [:project-config-filedesc :invalid?])
      ...
      (let [report-aliases-status (when (get-in cli-opts [:options :reports])
                                    (build-tasks-report-aliases/scan-alias
                                     monorepo-project-map
                                     verbose))
            fw-status (when (get-in cli-opts [:options :reports])
                        (build-tasks-reports-fw/report-monorepo
                         monorepo-project-map))
            linter-status (when (get-in cli-opts [:options :lint])
                            (build-linter/lint (:deps monorepo-project-map)
                                               verbose))
            commit-status
            (when commit-msg
        ...
        (when-not (nil? commit-status)
          (build-tasks-commit/synthesis commit-status))
