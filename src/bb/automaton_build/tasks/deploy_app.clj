(ns automaton-build.tasks.deploy-app
  ...
  (:require
   ...
   [automaton-build.echo.headers            :refer [errorln h1 h2 h2-error h2-valid normalln]]
...
(defn run
  ...
  (let [app-dir ""
    ...
    (let [{:keys [status run-id]}
      ...
      (h2 "Publish documentation")
      (when-not (= 0
                   (-> ["bb" "docs" tag "-r"]
                       (blocking-cmd app-dir "Documentation has failed" verbose?)
                       :exit))
        (h2-error "Documentation publishing has failed")
        (System/exit build-exit-codes/cannot-execute))
      (h2-valid "Documentation published"))))
