...
(defn- gha-lconnect*
  ...
  (if-let [exit-code (and (build-cfg-mgt/clone-repo-branch tmp-dir repo-url repo-branch)
                          (some-> (build-github-action/make-github-action app-name
                                                                          tmp-dir
                                                                          ""
...
(defn exec
  ...
  (let [tmp-dir (build-files/create-temp-dir "gha_image")]
    (gha-lconnect* tmp-dir (:repo-url gha) (:repo-branch gha) app-name account tag)))
