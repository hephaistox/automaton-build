...
(defn- cli-opts-spec-register
  ...
  {:force {:spec [["-f" "--force" "Force execution on local machine"]]}
   :tag {:spec [["-t" "--tag TAG-MESSAGE" "Tag for the publication"]]
         :mandatory? true}
   :environment {:spec [["-e" "--environment ENV-NAME" "Environment name for the publication"]]
                 :mandatory? true}
   :message {:spec [["-M" "--message COMMIT-MESSAGE" "Mandatory: Commit message"]]
             :mandatory? true}
   :message-opt {:spec [["-m" "--message-opt COMMIT-MESSAGE" "Commit message"]]}})

(defn- print-error-message
  [cli-opts msg]
  (build-terminal-msg/println-msg (format (cond
                                            (str/blank? msg) "The cli arguments are invalid"
                                            :else msg)))
