...
(defn- current-branch-not-base [current-branch base-branch] (not (= current-branch base-branch)))
  ...
  (let [current-branch (build-cfg-mgt/current-branch app-dir)]
    (when (current-branch-not-base current-branch base-branch)
      (let [res (build-cfg-mgt/pull-changes base-branch app-dir)]
        (build-log/trace-format "Merge of branch `%s` to branch `%s`, has resulted with: `%s`"
                                base-branch
                                current-branch
                                res)
...
(defn exec
  ...
  (let [base-branch (get-in publication [:env :production :push-branch])]
    (if (pull-base-branch app-dir base-branch)
      (do (build-log/debug "Current branch is up-to-date with base.") build-exit-codes/ok)
