...
(defn- commit
  ...
  (let [current-branch (build-cfg-mgt/current-branch app-dir)]
    (if-not (base-branch-push-disallowed current-branch main-branch)
      (ffirst
       (build-cfg-mgt/commit-and-push app-dir message-opt current-branch))
      false)))
...
(defn exec
  ...
  (if (nil? message-opt)
    (do (build-terminal-msg/println-msg
         "Committing skipped, due to missing -m option.")
        build-exit-codes/ok)
    (let [res (commit (get-in publication [:env :production :push-branch])
                      app-dir
                      message-opt)]
      (cond
