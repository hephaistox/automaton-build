...
(defn start-conf
  []
  (try
    (build-log/debug "Starting configuration")
    (let [files (build-conf-files/make-files-conf) env (build-conf-env/make-env-conf)] [files env])
    (catch Throwable e
      (build-log/fatal-exception (ex-info "Configuration failed, application will stop" {:error e}))
      (throw e))))
  ...
  ([key-path default-value]
   (if (not (vector? key-path))
     ...
     (let [value (or (build-conf-prot/read-conf-param (first (conf-state)) key-path)
       ...
       (if (nil? value)
         (do (build-log/trace-format "Read key-path %s returned nil, defaulted to `%s`"
                                     key-path
                                     default-value)
             default-value)
